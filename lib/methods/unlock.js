...
module.exports = function (hLock, options) {
  ...
  function _countFailures(kind, lockId, attempterId) {
    ...
    return Promise.resolve(UnlockFailure.count(query));
  }
  ...
  function _clearFailures(lockId, attempterId) {
    ...
    return Promise.resolve(UnlockFailure.update(query, updateData, updateOptions));
  }
  ...
  return function (lockId, secret, attempterId) {
    if (!ObjectId.isValid(lockId)) { return Promise.reject(new hLock.errors.InvalidLockId('lockId is required')); }
