...
module.exports = function (hLock, options) {
  ...
  return function (lockId, secret) {
    if (!lockId) { return Bluebird.reject(new hLock.errors.InvalidLockId('lockId is required for reseting')); }
    if (!secret) { return Bluebird.reject(new hLock.errors.InvalidLockSecret('lock secret is required for reseting')); }
