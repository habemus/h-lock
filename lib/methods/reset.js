...
module.exports = function (hLock, options) {
  ...
  return function (lockId, secret) {
    if (!lockId) { return Promise.reject(new hLock.errors.InvalidLockId('lockId is required for reseting')); }
    if (!secret) { return Promise.reject(new hLock.errors.InvalidLockSecret('lock secret is required for reseting')); }
